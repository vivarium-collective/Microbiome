...
class FBA(Process):
    ...
    def ports_schema(self):
        return {
            ...
            "flux_bounds": {
                '_default': {reaction.id: (reaction.lower_bound, reaction.upper_bound) for reaction in self.model.reactions},
                '_emit': True,
                "_updater": "set"
            }
        }
    ...
    def next_update(self, timestep, state):
        for reaction_id, bounds in state['flux_bounds'].items():
            self.model.reactions.get_by_id(reaction_id).bounds = bounds
